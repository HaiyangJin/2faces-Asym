...
Note: `MASS::contr.sdif()` was used in this set of analysis. And the results were highly similar to those reported in the manuscript (where some of other contrast coding was used.)
...
`Con_Ali`, `Cue_Con_Ali`, and `Ali_Sam` were removed from extended model (`glmm_E1_resp_zcp`) due to that the variances they explained were smaller than 0.1%, making `glmm_resp_rdc`.
...
`Con_C`, `Cue_Con`, `Ali_C`, `Cue_Ali`, and `Intercept` were removed from the extended model.
...
```{r}
summary(rePCA(glmm_E1_resp_etd5))
```

```{r resp etd6 E1}
file_E1_resp_etd6 <- file.path(folder_lmm, "E1_Resp_lmm_etd6.RData")

# fit the etd6 model
if (!file.exists(file_E1_resp_etd6)) {
  ...
  # save(glmm_E1_resp_etd6, file = file_E1_resp_etd6)
} else {
  load(file_E1_resp_etd6)
}

print(summary(glmm_E1_resp_etd6), corr = FALSE)
```

#### The optimal model
```{r comapre etd and rdc E1 d}
# compare the extended and reduced model
anova(glmm_E1_resp_etd6, glmm_E1_resp_rdc, refit = FALSE)
```
...
`(Intercept)`, `Ali_C`, `Cue_Ali`, `Pro_C`, `Sam_Pro`, `Cue_Ali_Pro`, `Cue_C`, `Con_Ali_Sam`, `Cue_Con_Ali_Sam`, and `Con_Sam_Pro` were removed from extended model (`glmm_E2_resp_etd`) due to that the variances they explained were smaller than 1%, making `glmm_E2_resp_etd1`.
...
`Cue_Ali_Sam_Pro`, and `Sam_C` were removed from extended model (`glmm_E2_resp_etd1`) due to that the variances they explained were smaller than 1%, making `glmm_E2_resp_etd2`.
...
`Con_Sam`, and `Cue_Pro` were removed from extended model (`glmm_E2_resp_etd2`) due to that the variances they explained were smaller than 1%, making `glmm_E2_resp_etd3`.
...
# compare the extended and reduced model
anova(glmm_E2_resp_etd4, glmm_E2_resp_rdc, refit = FALSE)
```
...
`Cue_Ali`, `Ali_Pro`, `Con_Ali_Pro`, `Con_Pro`, and `Cue_Con_Pro` were removed from extended model (`glmm_E2_rt_zcp`) due to that the variance it explained was smaller than 0.1%, making `glmm_E2_rt_rdc`.
...
```{r}
summary(rePCA(glmm_E2_rt_rdc))
```
`Cue_Con_Ali` was removed. 

```{r E2 rt rdc1}
file_E2_rt_rdc1 <- file.path(folder_lmm, "E2_rt_lmm_rdc1.RData")

# fit the rdc1 model
if (!file.exists(file_E2_rt_rdc1)) {
  ...
  # save(glmm_E2_rt_rdc1, file = file_E2_rt_rdc1)
} else {
  load(file_E2_rt_rdc1)
}

print(summary(glmm_E2_rt_rdc1), corr = FALSE)
```
```{r}
summary(rePCA(glmm_E2_rt_rdc1))
```
`Cue_Con_Ali_Pro` was removed. 

```{r E2 rt rdc2}
file_E2_rt_rdc2 <- file.path(folder_lmm, "E2_rt_lmm_rdc2.RData")

# fit the rdc2 model
if (!file.exists(file_E2_rt_rdc2)) {
  ...
  # save(glmm_E2_rt_rdc2, file = file_E2_rt_rdc2)
} else {
  load(file_E2_rt_rdc2)
}

print(summary(glmm_E2_rt_rdc2), corr = FALSE)
```
...
```
`Con_C`, `Ali_C`, `Cue_Con`, and `Con_Ali` were removed from extended model.
...
# compare the extended and reduced model
anova(glmm_E2_rt_etd1, glmm_E2_rt_rdc, refit = FALSE)
```
